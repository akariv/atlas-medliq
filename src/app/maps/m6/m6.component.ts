...
export class M6Component implements OnInit {
  ...
  constructor(private api: M6Service, private map: MapService, private player: PlayerService, private activatedRoute: ActivatedRoute) {
    api.getSamples().pipe(
      switchMap((samples) => {
        this.sample = samples.features[0].properties;
        this.name = this.sample.name;
        this.pics = this.sample.Images?[...this.sample.Images.map(image => image.width <= image.height ? { ...image, newClass: true } : { ...image, newClass: false })]: [];
        ...
        setTimeout(function () {
          Array.from(document.querySelectorAll('.notes a')).forEach(function (elem) {
  ...
  clicking(el) {
    const sampleToZoom = this.features.filter(({ properties }) => el.innerHTML.trim() === properties.name);
    ...
    this.name = this.sample.name;
    this.pics = this.sample.Images?[...this.sample.Images.map(image => image.width <= image.height ? { ...image, newClass: true } : { ...image, newClass: false })]: null;
    this.description = this.sample.description;
    this.Location = this.sample.Location;
    this.Chapter = this.sample.Chapter;
    this.Notes = this.sample.Notes;
    this.Shortcuts = this.sample.Shortcuts === undefined ?this.sample.Shortcuts?.json():this.sample.Shortcuts;
    this.title = this.Location + ' - ' + this.Chapter;
  ...
  ngOnInit(): void {
    ...
    this.theMap = new mapboxgl.Map({
      ...
      accessToken: 'pk.eyJ1IjoiYXRsYXNtZWRsaXEiLCJhIjoiY2tpbXgzNW5qMHhhcjJ5cGtydHpkNnJqYyJ9.vfietFuvTA8S1vaGlm3CUQ',
    ...
    this.theMap.on('style.load', () => {
      this.samples.pipe(first()).subscribe((samples: GeoJSON.FeatureCollection) => {
        ...
        const geojson = {
          ...
          'features': [
            ...samples.features.filter(({ properties }) => properties.Chapter === 'introduction')
          ]
        };
        for (const marker of geojson.features) {
          ...
          el.addEventListener('mouseenter', () => {
            ...
            this.Location = this.sample.Location;
            this.Chapter = this.sample.Chapter;
            popup.setLngLat(coordinates.reverse()).setHTML(this.Location + ' - ' + this.Chapter).addTo(this.theMap);
          ...
          el.addEventListener('click', () => {
            ...
            this.sample = this.sample.Images?{
              ...this.sample, Images: [
                ...this.sample.Images
                  .map(image => image.width <= image.height ? { ...image, newClass: true } : { ...image, newClass: false })],
        ...
        this.theMap.on('click', 'clusters', (e) => {
          ...
          const source: mapboxgl.GeoJSONSource = this.theMap.getSource('samples') as mapboxgl.GeoJSONSource
        ...
        this.theMap.on('mouseenter', this.LAYER_NAME, (e) => {
          ...
          this.Location = this.sample.Location;
          this.Chapter = this.sample.Chapter;
          popup.setLngLat(coordinates).setHTML(this.Location + ' - ' + this.Chapter).addTo(this.theMap);
        ...
        this.theMap.on('click', this.LAYER_NAME, (e) => {
          this.sample = e.features[0].properties;
          this.sample = {
            ...this.sample, Images: [...JSON.parse(this.sample.Images)
              .map(image => image.width <= image.height ? { ...image, newClass: true } : { ...image, newClass: false })],
          ...
          this.Chapter = this.sample.Chapter;
          this.Notes = this.sample.Notes;
          this.Shortcuts = this.sample.Shortcuts === undefined ?this.sample.Shortcuts?.json():JSON.parse(this.sample.Shortcuts);
          this.title = this.Location + ' - ' + this.Chapter;
          this.features = samples.features;
